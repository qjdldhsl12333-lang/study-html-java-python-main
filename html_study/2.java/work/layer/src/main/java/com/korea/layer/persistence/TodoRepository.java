...
@Repository
//데이터베이스와 상호작용하는 클래스에서 사용되며,CRUD와 같은 데이터베이스 작업
//을 처리하는데 사용된다. Spring 의 Bean으로 등록된다.
...
public interface TodoRepository extends JpaRepository<TodoEntity, String>{
	...
	//쉽게 처리할 수 있도록 도와주는 역활을 한다.
	//T : 엔티티 클래스를 의미한다.
	//ID : 엔티티 클래스의 기본키의 타입을 의미한다.
	
	//주요 메서드
	//save(S entity) : 엔티티를 데이터 베이스에 저장하거나 업데이트 한다.
	//findById(ID id) : 기본키를 이용하여 엔티티를 조회한다.
	//findAll() : 데이터 베이스의 모든 엔티티를 조회한다.
	//deleteById(ID id) : 기본키를 이용하여 엔티티를 삭제한다.
	
	//많이 쓰는 접두사
	//findBy
	//By 뒤에 오는 부분 -> 엔티티의 필드명 기준 조건
	...
	//기본으로 제공하지 않는것들은 인터페이스에 추상 메서드로 만들어서 사용해야한다.
