...
public class PatternExample {
	...
	//사용법 : [] > 문자 : 일치하는 문자가 1개 있는지 검증 
	//ex) [abc] > a,b,c 만 허용하는 한글자???
		//문자 범위
		//0-9 : 0~9 중 숫자 하나.
		//a-z : 소문자 중 하나
		//A-Z : 대문자 중 하나
		...
		//자주 쓰이는 패턴 
		//^...$
		//ex)^[0-9]+$ : 전체 문자열에서 숫자가 1 개 이상이여야 한다.
	String reg = "[\\w]+";
	String data = "hello";
	//Pattern 클래스
	//matches (정규표현식, 데이터);
	...
	//[a-zA-Z0-9]{3,11}$ : 영문 또는 숫자가 3~11 글자를 만족해야한다.
	String id = "gildong123";
	if(Pattern.matches(regex, id)) {
		System.out.println("아이디형식이 올바릅니다");
	}else {
