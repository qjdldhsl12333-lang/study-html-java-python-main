...
public class exam {
	public static void main(String[] args) {
		...
		List<String> fruit = words.stream()
	    ...
	    double avg = students.stream()
	    		...
	    		.average()//평균을 계산을 하라
	    		.orElse(0.0); // 값이 없으면 기본값 0.0을 반환하라
	    System.out.println(avg);
	    
	    //students.stream() -> Stream<Student>
	    //평균,합계,최대값같은 "숫자연산"을 하고 싶을 때는 정수 스트림(IntStream)이 필요하다.
		
		//점수 구간별 그룹핑
		//90점 이상 -> key "A"
		//80점 이상 -> key "B"
		//70점 이상 -> key "C"
		Map<String ,List<Student>> frouped = students.stream()
