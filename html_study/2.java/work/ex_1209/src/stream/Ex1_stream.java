...
public class Ex1_stream {
	public static void main(String[] args) {
		...
		//중간연산(내가 원하는 걸로 가공)
		List<Integer> nums = List.of(5,1,3,2,4,6,7,8,1,8,1,9,7,8,2,0,8,5,6,8,7,5);
		...
		//스트림으로 가공한 결과를 List,Set같은 컬렉션으로 다시 만들고 싶으면
		//Collection(Colletors...)를 사용한다.
			//짝수 리스트를 만들되 오름차순 정렬을 하고 중복을 제거하시오
		...
		//Colllectors에 들어있는 메서드
		//toList() : 리스트로 반환
		//toSet() : Set으로 반환
		//joining(",") : 문자열들을 하나로 합치기
		//groupingBy(...) : 특정 기준으로 그룹나누기 
		...
		//joining
		List<String> words = List.of("Java","Spring","React","Python");
		String joined = words.stream()
				.collect(Collectors.joining(","));
		System.out.println(joined);
		
		List<String> names2 = List.of("kim","lee","park","choi","jin");
		Map<Integer, List<String>> groued =
				names2.stream()
		...
		System.out.println("짝수의 갯수 : " + count2);
