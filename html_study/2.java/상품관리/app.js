...
function renderTable(products){
    ...
    //라디오 버튼에 change 이벤트 리스너 등록
    document.querySelectorAll('input[name="selectedProduct"]')
        .forEach(radio => {
...
function addNewRowToTable(item){
    ...
    row.insertCell(6).textContent = item.registerTime;
    row.insertCell(7).textContent = item.updateTime;
}
...
function handleRadioChange(event){
    document.querySelectorAll('.order-count-input')
        .forEach(input => {
        input.disabled = true; // 모든 입력 필드를 비활성화
        input.value = ''; // 입력 필드 초기화
    });
    //선택된 라디오 버튼의 상품ID를 가져온다.
    const selectedProductId = event.target.value;
    //선택된 상품 ID 에 해당하는 입력필드를 찾아서 활성화
    const selectedInput = document.querySelector(`#orderCount-${selectedProductId}`);
    if(selectedInput){
        selectedInput.disabled = false;
        selectedInput.focus(); //포커스 설정
    }
    //주문하기
...
//서버로 넘기기위한 데이터를 가져온다
        ...
        //진짜 숫자를 입력했는지,0이나 음수를 입력했는지 조사
        if(isNaN(productCount) || productCount < 0){
            orderMessageElement.textContent = "올바른 주문 개수를 입력해주세요.";
            orderMessageElement.style.color = 'orange';
            orderInput.focus();
            return;
        }
        ...
        orderMessageElement.textContent = "주문 처리 중...";
        orderMessageElement.style.color = '#007bff';

        try {
            //서버에 요청
            const updateProduct = await apiFetch('/orders/create',{
