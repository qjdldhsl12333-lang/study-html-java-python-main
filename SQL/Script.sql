...
create table TBL_MEMBER(
	NAME varchar(30), -- varchar문자열 데이터만 들어갈수 있음.
...
#테이블의 변경 
#create 문으로 테이블을 생성한 후 불가피하게 수정해야 할 상황이 발생할 때가 많다.
#최초 설계를 잘못한 원인도 있고, 요구사항이 변경되기 때문이기도 하다.(프로젝트상)
...
#컬럼명 바꾸기 rename
alter table ex01 rename column fisrt_col to first_col;

#컬럼 타입변경 modify
alter table ex01 modify column col3 int;

#제약조건 추가 add 제약조건(컬럼명);
